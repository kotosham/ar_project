...
FRESHNESS = {
    ...
    # этот модуль.
    'heartbeat_sim': 12.0,
    # Та же болезнь у /robot_health, и по той же причине. Агрегатор публикует
    # по таймеру rclpy 1 Гц с use_sim_time=True — то есть раз в одну
    # СИМУЛЯЦИОННУЮ секунду, — а консоль меряет возраст стенными часами
    # (console_node: time.monotonic). При RTF<1 нормальный интервал прихода в
    # стенных секундах равен 1/RTF: при RTF≈0.5 это уже 2 с, и одного
    # проскользнувшего тика хватает, чтобы перевалить 3.0 с. Строка 'health' не
    # advisory, поэтому такой промах сразу давал блокирующее «НЕ ГОТОВ» при
    # полностью живом агрегаторе. На железе use_sim_time=False, часы общие,
    # порог остаётся 3.0.
    'health_sim': 12.0,
    'camera': 3.0,
    'scan': 3.0,
...
PROFILES = {
    SIM: {
        ...
        # ОБА пути детектора на GroundingDINO, YOLOE в симуляции не поднимается.
        ...
        # находки, GroundingDINO нашёл 10. Мебель здесь — некрашеные примитивы
        ...
        #
        # ЧЕСТНО ПРО DETECT_ALL: словарный путь здесь будет возвращать мало или
        # ничего, и это не поломка, а мера. Замерено: каждый из 12 классов
        # срабатывает на каждом кадре с 0.2..0.8, argmax совпадает с истиной 2
        # раза из 11. GroundingDINO отвечает «здесь что-то есть», а не «что
        # именно», когда предмет — некрашеный примитив. Поэтому segment_vocab
        # требует отрыва победителя и молчит, когда отрыва нет. Настоящее
        # лекарство — текстурированная мебель в мире, а не смена детектора.
        'detector_model_mode': 'dino_mobilesam',
