...
**Единый канал камеры (анти-fan-out).** Камера пересекает Wi-Fi ровно один раз:
Pi отдаёт `compressed` (JPEG) + `compressedDepth` + `camera_info`, а на edge
`edge_camera_relay.launch.py` — **единственный** Wi-Fi-подписчик камеры —
декомпрессирует поток один раз и переиздаёт его edge-локально на `/camera_edge/*`.
SLAM, детектор и VLM-оркестратор подписываются только на `/camera_edge/*`, поэтому
каждый дополнительный потребитель на edge стоит **ноль** дополнительной полосы линка.
Прямые подписки edge-нод на `/camera/camera/*` запрещены — каждая такая подписка
открывает свой собственный (в худшем случае сырой) поток с Pi.
