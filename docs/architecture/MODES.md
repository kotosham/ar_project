...
- **`vlm`** — над исполнителем работает **Planner Orchestrator** (на edge/PC, асинхронный клиент к Qwen3-VL-30B-A3B через vLLM, OpenAI-совместимый API). Он **не управляет роботом напрямую и не выдаёт координат навигации**. Он лишь **декомпозирует высокоуровневую инструкцию в последовательность/дерево подзадач**, каждая из которых — это **обычный FLAT-skill**. Текущую подзадачу диспетчеризует FLAT-исполнителю и **периодически перепланирует** по накопленной истории.
...
```
Оператор          Orchestrator (edge)            Executive (Pi, FLAT)         VLM (vLLM)
   │ instruction        │                              │                          │
   ├───────────────────▶│ decompose                    │                          │
